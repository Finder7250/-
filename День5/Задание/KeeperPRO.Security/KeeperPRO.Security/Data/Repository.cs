...
namespace KeeperPRO.Security.Data
{
    public class Repository
    {
        ...
        public DataTable GetApprovedRequestsFiltered(DateTime? date, string type, int? departmentId)
        {
            ...
            if (date.HasValue)
            {
                query += string.Format(" AND r.start_date::DATE = '{0:yyyy-MM-dd}'", date.Value);
            }
            ...
            {
                query += " AND r.department_id = " + departmentId.Value;
            }
        ...
        {
            string query = "UPDATE visitrequests SET access_start_time = @accessTime WHERE request_id = @requestId";
            using (var conn = new NpgsqlConnection(_connectionString))
            using (var cmd = new NpgsqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@accessTime", accessTime);
                cmd.Parameters.AddWithValue("@requestId", requestId);
                conn.Open();
        ...
        {
            string query = "UPDATE visitrequests SET access_end_time = @departureTime WHERE request_id = @requestId";
            using (var conn = new NpgsqlConnection(_connectionString))
            using (var cmd = new NpgsqlCommand(query, conn))
            {
